...
#define VERSION "EasySDR Firmware V1.0 xiaolaba testing"
...
void setup()
{
  ...
  // initialize the digital pin as an output for on-board LED.
  LED_setup();
...
void set_rx_freq(uint32_t freq)
{
    ...
    ledcAttachPin(OUTPIN_A, I_CHANNEL);   // swap my IQ signal to match hardware, xiaolaba
    ledcAttachPin(OUTPIN_B, Q_CHANNEL);	
  ...
  pinMode(OUTPIN_A, INPUT); // change name to match my circuit, xiaolaba
  pinMode(OUTPIN_B, INPUT);
...
{
    //LED_blinking();
    slowBlink (); // sigal the user for running status
}
